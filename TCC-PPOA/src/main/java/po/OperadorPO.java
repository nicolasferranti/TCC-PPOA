 ...
 */
public class OperadorPO {

    /**
     * Realiza reproducao com ponto de cruzamento Ãºnico. Pega 50% de cada um
     * dos pais. Se tiver um nÃºmero Ã­mpar de cromossomos (3 por exemplo, pega
     * 2 de um e 1 de outro);
     *
     * @param candidato1
     * @param candidato2
     * @return filho gerado
    ...
    public static Cromossomo reproducaoPontoCruzamento(Cromossomo candidato1, Cromossomo candidato2, int numGenes) {
        ...
        if (res == 0) {
            ...
            for (int k = 0; k < (div); k++) //Então parece que essa é a parte critica!!!
            ...
            for (int l = div; l < numGenes; l++) //filho.addGene(candidato2.getGene(l));
            {
            ...
            for (int k = 0; k < (div + 1); k++) //filho.addGene(candidato1.getGene(k));
            {
                filho.addGene(new Gene(candidato1.getGene(k).getValor()));
            }

            for (int l = div + 1; l < numGenes; l++) //filho.addGene(candidato2.getGene(l));
            {
                filho.addGene(new Gene(candidato2.getGene(l).getValor()));
            }
        }
    ...
    public static BigDecimal avaliaCromossomo(Cromossomo c, List<IFuncao> equacoes, double resultadoEquacoes[]) {
        ...
        for (int i = 0; i < equacoes.size(); i++) {
            ...
            //System.out.println(equacaoAux.getFuncao());
            ...
            //System.out.println("Soma da linha "+ (i+1) +" : "+ somaAux);
    ...
    public static BigDecimal[] avaliaCromossomoVet(Cromossomo c, List<IFuncao> equacoes, double resultadoEquacoes[]) {

        BigDecimal soma = new BigDecimal(0);
        Equacao equacaoAux;
        
        BigDecimal[] resultados = new BigDecimal[equacoes.size()];
        for (int i = 0; i < equacoes.size(); i++) {
            BigDecimal somaAux = new BigDecimal(0);
            equacaoAux = new Equacao(equacoes.get(i));
            //System.out.println(equacaoAux.getFuncao());
    ...
    /**
     * A mutação sempre soma a granularidade na posição escolhida Caso o gene
     * possua valor maior que 1, subtraia
     *
     *
     */
